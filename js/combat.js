...
import { Mage } from './mage.js?v=mage1';
...
export class Combat {
  constructor(game) {
    ...
    this.panel.innerHTML = '<button id="equipAK">切换职业 [Q]</button><button id="fireAK">攻击</button><button id="blinkMage">闪现 [Shift]</button><button id="buildMode">建造 [B]</button><span id="combatStatus"></span>';
  ...
  tick() {
    ...
    this.status.textContent = g.player.hp <= 0 ? `已阵亡 · ${Math.max(1, Math.ceil((this.deadUntil - Date.now()) / 1000))} 秒后重生` :
      ...
      this.armed ? 'AK · 按住左键连射 · Q 切法师 · B 建造' : '建造 · Q 装备 AK，再按 Q 切法师';
